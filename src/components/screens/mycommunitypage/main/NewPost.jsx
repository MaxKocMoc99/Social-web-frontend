...
const NewPost = ({g}) => {
  ...
  const { setWrapper, setAllPosts, users, me } = useContext(WrapperContext)
  ...
  const doPost = async () => {
    await axios.post("http://localhost:8080/posts/addGroupPost", {
      name: g.name,
      time: hours + ':' + minutes,
      text: text,
      img: imageUrl,
      ...
      <div className="main__newpost__buttons">
        ...
        <MdOutlinePhotoCamera onClick={() => {
          ...
          setWrapper('darkwrapper')
              }} cursor={'pointer'} color={'gray'} size={20}/>
      </div>
